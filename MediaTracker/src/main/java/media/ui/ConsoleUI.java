...
public class ConsoleUI {
	...
	public ConsoleUI(CategoryService serv, ItemService itemServ, RatingService ratingServ, TagService tagServ,
			Scanner scanner) {
		this.serv = serv;
	...
	// Helper Method for Update Category, allows user to enter name to find Category
	// instead of ID
	...
	public void ratingStart() throws SQLException { 
		int ratingChoice = 0;
		while(ratingChoice != 7) {
		System.out.println("Manage Ratings: ");
		System.out.println("1. Add Rating");
		System.out.println("2. View Rating by Item");
		System.out.println("3. View All Ratings by Item");
		System.out.println("4. Update Score"); 
		System.out.println("5. Update Review");
		System.out.println("6. Delete Rating");
		...
		switch(choice) {
			case 1: {
				//Add Rating
				System.out.println("Enter a new Rating Score (1-10): ");
				int ratingScore = scanner.nextInt();
				scanner.nextLine();
				if(ratingScore <= 10 && ratingScore >= 1) {
					System.out.println("Enter a review (Optional): ");
					...
					System.out.println("Enter a Item Title to add this Rating to: ");
					String itemTitle = scanner.nextLine();
					Item foundItem = findItemByName(itemTitle, items);
					if(foundItem == null) {
						System.out.println("Item Title not found");
					} else {
						Rating newRating = new Rating(foundItem.getItemId(), ratingScore, review);
						ratingServ.addRating(newRating);
					}
				} else {
			...
			case 2: {
				...
				if(foundItem == null) {
					...
					List<Rating> ratings = ratingServ.getRatingsForItem(foundItem.getItemId());
					for(Rating r : ratings)
					System.out.println(foundItem.getTitle() + " " + r.toString());
				}
				break;
			}
			case 3: {
				...
				List<Item> items = itemServ.getAllItems();
				for(Item i : items) {
					System.out.println(i.getTitle() + " " + ratingServ.getRatingsForItem(i.getItemId()));
				} 
				break;
			}
			case 4: {
				...
				System.out.println("Enter an Item Title to Update Score of: ");
				String titleName = scanner.nextLine();
				Item foundItem = findItemByName(titleName, items);
				if(foundItem == null) {
					System.out.println("Item not found");
				} else {
					System.out.println("Enter a Rating ID to Update: ");
					int ratingID = scanner.nextInt();
					scanner.nextLine();
					System.out.println("Enter a New Score: ");
					int score = scanner.nextInt();
					scanner.nextLine();
					ratingServ.updateScore(ratingID, score);
				}
				break;
			}
			case 5: {
				...
				System.out.println("Enter an Item Title to Update Review of: ");
				String titleName = scanner.nextLine();
				Item foundItem = findItemByName(titleName, items);
				if(foundItem == null) {
					System.out.println("Item not found");
					...
					int ratingId = scanner.nextInt();
					scanner.nextLine();
					System.out.println("Enter a New Review or Enter Blank to Remove: ");
					String review = scanner.nextLine();
					if(review.isBlank()) {
						review = null;
					}
						ratingServ.updateReview(ratingId, review);
					}
				break;
			}
			case 6: {
				...
				System.out.println("Enter an Item Title to Remove Rating on: ");
				String titleName = scanner.nextLine();
				Item foundItem = findItemByName(titleName, items);
				if(foundItem == null) {
					System.out.println("Item not found");
				} else {
					System.out.println("Enter a Rating ID to Remove: ");
					int ratingId = scanner.nextInt();
					scanner.nextLine();
					ratingServ.removeRating(ratingId);
				}
			...
			default: {
				System.out.println("Must enter an option 1-7");
			}
		}
		}
	}
