...
I built this project to practice connecting Java to a real database after learning SQL. The idea is simple — a personal tracker where you can rate anything you want across categories you define yourself. Movies, food spots, shoes, games, whatever. I got tired of seeing the same "todo list" or "bank account" Java projects everywhere, so I wanted something I'd actually use.

---

## What it does

You create your own categories (Movies, Food, Books — anything), add items to them, tag them however you want, and rate them with a score and optional review. You can track the status of anything too — whether it's something you want to check out, something you're currently in the middle of, or something you've already finished.

The whole thing runs as a console app with a menu system. Not the prettiest, but it hits everything I wanted to practice: real database design, JDBC, layered architecture, and Git workflow from day one.
...
The persistence layer uses the Repository pattern — each entity (Category, Item, Tag, Rating) has its own interface and JDBC implementation. The service layer sits on top and handles validation, so invalid data gets caught in Java before it ever reaches the database. The UI only talks to services, never to repositories directly.
